...
test_save_path = '/data/hsx/project_pc_multimodality_addsize/Task_Pancreas_sec/raw_data' #'F:\Experiment_Data\Task_Pancreas_Series\implement_baseline\sd_niigz'
...
def verify_same_geometry(img_1: sitk.Image, img_2: sitk.Image, case_name):
    ...
    if not same_ori:
        ...
        print(ori1)
        print(ori2)
        print("try to match mask origin with image and now:")
        img_2.SetOrigin(img_1.GetOrigin())
        print(img_1.GetOrigin())
        print(img_2.GetOrigin())
        reorigin_mask_file = case_name+'.nii.gz' # 10194607_0_a
        sitk.WriteImage(img_2, join(test_save_path, 'masks', reorigin_mask_file))
    ...
    if not same_spac:
        ...
        print(spacing1)
        print(spacing2)
        print("try to match mask spacing with image and now:")
        resample_img_2 = sitk.Resample(img_2, img_1.GetSize(), sitk.Transform(), sitk.sitkNearestNeighbor,
                                             img_1.GetOrigin(), img_1.GetSpacing(), img_1.GetDirection(), 0.0,
                                             img_2.GetPixelIDValue())
        print(img_1.GetSpacing())
        print(resample_img_2.GetSpacing())
        resample_mask_file = case_name+'.nii.gz' # 10194607_0_a
        sitk.WriteImage(resample_img_2, join(test_save_path, 'masks', resample_mask_file))
    ...
    if not same_dir:
        ...
        print(direction1)
        print(direction2)
        print("try to match mask direction with image and now:")
        img_2.SetDirection(img_1.GetDirection())
        print(img_1.GetDirection())
        print(img_2.GetDirection())
        redirection_mask_file = case_name+'.nii.gz' # 10194607_0_a
        sitk.WriteImage(img_2, join(test_save_path, 'masks', redirection_mask_file))
...
def verify_dataset_integrity(folder):
    ...
    assert isdir(join(folder, "images")), "There needs to be a imagesTr subfolder in folder, folder=%s" % folder
    assert isdir(join(folder, "masks")), "There needs to be a labelsTr subfolder in folder, folder=%s" % folder
    ...
    nii_files_in_labelsTr = subfiles((join(folder, "masks")), suffix=".nii.gz", join=False)
    ...
    for c in expected_train_identifiers:
        ...
        expected_label_file = join(folder, "masks", c + ".nii.gz")
        label_files.append(expected_label_file)
        expected_image_files = [join(folder, "images", c + ".nii.gz")] # _%04.0d
        ...
        for i, img in enumerate(images_itk):
            ...
            same_geometry = verify_same_geometry(img, label_itk, c)
            if not same_geometry:
                ...
                print("The geometry of the image %s does not match the geometry of the label file. The pixel arrays "
                      ...
                      "are coregistered and have the same geometry as the label" % expected_image_files[0][:-17])
            if nans_in_image:
    ...
    # check test set, but only if there actually is a test set 这里没有修改 因为所有文件都纳入训练集中检查
